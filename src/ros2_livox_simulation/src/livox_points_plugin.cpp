...
namespace gazebo
{
    ...
    void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent, sdf::ElementPtr sdf)
    {
        ...
        if (sdfPtr->HasElement("scan_period_sec"))
        {
            scanPeriodSec = sdfPtr->Get<double>("scan_period_sec");
        }
        if (scanPeriodSec <= 0.0)
        {
            scanPeriodSec = 0.1;
        }
        if (downSample < 1)
    ...
    void LivoxPointsPlugin::OnNewLaserScans() {
        ...
        pp_livox.timebase = static_cast<std::uint64_t>(stamp.nanoseconds());
        pp_livox.lidar_id = 0;
        int count = 0;
        const auto point_count = std::max<std::size_t>(1, points_pair.size());
        const double scan_period_ns = scanPeriodSec * 1e9;
        ...
        for (const auto &pair : points_pair) {
            ...
            // The custom ODE MultiRayShape does not reliably expose the SDF
            // range bounds through RangeMin()/RangeMax(). Use the bounds we
            // parsed directly from <ray><range> so invalid/no-hit values can
            // never leak hundreds of metres into LIO preprocessing.
            if (!std::isfinite(range) || range <= minDist || range >= maxDist) {
                range = 0.0;
            }
            ...
            *out_z = point.Z();
            ...
            // Gazebo ray evaluation is effectively a frame scan. Represent it as
            // a deterministic Livox-style scan period instead of CPU loop time,
            // so LIO deskew receives monotonic per-point timing.
            p.offset_time = static_cast<std::uint32_t>(
                scan_period_ns * static_cast<double>(count) /
